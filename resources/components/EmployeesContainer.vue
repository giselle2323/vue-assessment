<template>
  <section class="employee-container">
    ...
    <section v-if="!employees.length" class="employee-container__table">
      <h2 class="no-data-text">Sorry , no data available at the moment, create a new employee to manage you data</h2>
    </section>
        ...
        <table class="table table-responsive">
          ...
          <tbody>
            <tr v-for="employee in employees" :key="employee.id">
              <td class="checkbox-row"><input type="checkbox" name="checkbox" :id="employee.id" /></td>
              <td class="image-row" ><img :src="employee.image" alt="profile" height="45px" width="45px"/></td>
              <td class="name-row"><p class="bold-name-column">{{ employee.name }}</p> <br> <p class="light-column">{{ employee.job_title}} </p></td>
              <td class="salary-row">  <p class="bold-column"> {{ employee.salary }} NOK 
                  <span class="gain" v-if="employee.last_salary_change >= 0 "> + {{ employee.last_salary_change}}</span>
...
export default {
  ...
  methods: {
    ...
    deleteEmployee(id) {
      let deleteData = confirm("Are you sure you want to delete this employee?");
      if (deleteData == true) {
